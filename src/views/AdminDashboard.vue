          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr v-for="order in orders" :key="order.id">
                ...
                <td class="font-mono text-dark-600 dark:text-dark-400">
                  {{ order.target_number }}
                </td>
                <td>
              ...
              <tr v-for="u in usdtConversions" :key="u.id">
                <td>
                  ...
                  <div v-if="u.bank_details" class="bank-info">
                    <div class="font-bold text-dark-950 dark:text-white">
                      {{ u.bank_details.bank_name }}
                    </div>
                    <div class="font-semibold text-dark-700 dark:text-dark-300">
                      {{ u.bank_details.account_number }}
                    </div>
                    <div class="text-dark-500 dark:text-dark-400 text-xs">
                      a.n. {{ u.bank_details.account_name }}
                    </div>
                  </div>
                </td>
                <td>
                  ...
                  <div class="text-xs text-dark-500 dark:text-dark-400 mt-1">
                    ≈ Rp {{ formatPrice(u.idr_received) }}
                  </div>
                </td>

                <!-- ✅ FIX: Bukti Transfer via authenticated endpoint -->
                <td class="text-center">
                  <button
                    v-if="u.proof_path && !u.proof_blob"
                    @click="loadProof(u)"
                    class="action-link"
                  >
                    Lihat Bukti
                ...
                <td class="text-center">
                  <div v-if="u.status === 'pending'" class="flex gap-2 justify-center">
                    <button 
                      @click="openApproveUsdtModal(u, 'approved')"
                      class="action-btn success"
                    >
                      Setujui
                    </button>
                    <button 
                      @click="openApproveUsdtModal(u, 'rejected')"
                      class="action-btn danger"
                    >
                      Tolak
                    </button>
                  </div>
                  <span v-else class="status-badge" :class="u.status">
                    {{ u.status }}
                  </span>
                </td>
              </tr>
      ...
      <Transition name="modal">
        <div v-if="showEditPriceModal" class="modal-overlay" @click.self="closeEditPriceModal">
          <div class="modal-content">
            ...
            <input 
              v-model="newSellingPrice" 
              type="number"
              class="modal-input"
              placeholder="Masukkan harga baru" 
            />
            <div class="modal-actions">
              <button @click="closeEditPriceModal" class="modal-btn secondary">
                Batal
              </button>
              <button @click="saveNewPrice" class="modal-btn primary">
                Simpan
              </button>
            </div>
          </div>
...
import { ShoppingBag, Clock, TrendingUp, RefreshCw, Loader, Check, Zap } from 'lucide-vue-next'
...
// ✅ BULK MARGIN: Set margin seragam ke semua produk
...
const handlePinConfirmed = async (pin) => {
  ...
  try {
    ...
    // ✅ Handle bulk margin setelah PIN confirmed
...
// ✅ Load bukti transfer dari private storage via authenticated endpoint
...
.dark .card-table thead {
  ...
  border-bottom-color: rgb(42 49 66);
}
