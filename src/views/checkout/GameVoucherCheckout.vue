  ...
  <div class="game-page">
    ...
    <div class="game-header">
      <div class="header-inner">
        ...
        <div>
          <h1 class="header-title">Voucher Game</h1>
          <p class="header-sub">Beli voucher game favorit</p>
        </div>
      </div>
    </div>
    ...
    <div class="page-content">
      ...
      <div v-if="!selectedGame">
        ...
        <div v-if="productStore.loading" class="game-grid">
          <div v-for="i in 6" :key="i" class="game-card-skeleton" />
        </div>
          ...
          <button v-for="game in filteredGames" :key="game.brand" @click="selectGame(game)" class="game-card">
            <div class="game-card-bg">
              <img :src="getGameBanner(game.brand)" :alt="game.label" class="game-card-img"
                @error="(e) => e.target.src='/logos/games/'+game.brand.toLowerCase().replace(/ /g,'')+'.png'" />
        ...
        <div v-else class="empty-state">
          ...
          <p class="empty-title">Voucher tidak ditemukan</p>
      ...
      <div v-else>
        <div class="selected-game-header">
          ...
          <div class="selected-game-info">
            ...
            <div>
              ...
              <p class="selected-game-sub">{{ gameProducts.length }} voucher tersedia</p>
            </div>
          </div>
        </div>

        <!-- Info voucher dikirim ke email -->
        ...
        <div v-else-if="gameProducts.length > 0">
          <h2 class="section-title">Pilih Nominal Voucher</h2>
          <div class="product-grid">
            <button v-for="product in gameProducts" :key="product.id"
              ...
              <div class="product-icon">
                <Ticket :size="20" class="ticket-icon" />
              </div>
        ...
        <div v-else class="empty-state">
          ...
          <p class="empty-title">Voucher belum tersedia</p>
        </div>
      </div>
    </div>

    <PaymentModal v-if="selectedProduct" :product="selectedProduct"
      customer-data="VOUCHER" customer-label="Tipe"
...
const GAME_ASSETS = {
  ...
  'AXIS':                { banner: '/images/games/default.png', logo: '/logos/games/axis.png',            label: 'AXIS' },
  'XL':                  { banner: '/images/games/default.png', logo: '/logos/games/xl.png',              label: 'XL' },
}
...
const allGames = computed(() => {
  const map = {}
  productStore.products.filter(p => isVoucherCategory(p.category)).forEach(p => {
    if (!p.brand) return
...
const gameProducts = computed(() => {
  ...
  return productStore.products
    .filter(p => isVoucherCategory(p.category) && p.brand === selectedGame.value.brand)
...
.search-clear { position:absolute; right:12px; top:50%; transform:translateY(-50%); color:var(--muted-foreground,#6b7280); background:none; border:none; cursor:pointer; display:flex; align-items:center; padding:4px; border-radius:6px; }
.cat-tabs { display:flex; gap:8px; margin-bottom:20px; overflow-x:auto; scrollbar-width:none; }
.cat-tabs::-webkit-scrollbar { display:none; }
.cat-tab { padding:8px 18px; border-radius:999px; font-size:0.8125rem; font-weight:600; color:var(--muted-foreground,#6b7280); background:var(--card,#ffffff); border:1px solid var(--border,#e5e7eb); cursor:pointer; white-space:nowrap; transition:all 0.2s; }
.cat-tab:hover { color:var(--foreground,#111827); border-color:#16a34a; background:rgba(22,163,74,0.05); }
.cat-tab.active { background:#16a34a; color:#ffffff; border-color:#16a34a; box-shadow:0 2px 8px rgba(22,163,74,0.3); }
...
.game-card { position:relative; aspect-ratio:3/4; border-radius:16px; overflow:hidden; cursor:pointer; border:1px solid var(--border,#e5e7eb); background:var(--card,#ffffff); transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1); }
.game-card:hover { transform:translateY(-6px) scale(1.02); box-shadow:0 20px 40px rgba(0,0,0,0.12); border-color:#16a34a; }
...
.game-card-overlay { position:absolute; inset:0; background:linear-gradient(to bottom,rgba(0,0,0,0.05) 0%,rgba(0,0,0,0) 40%,rgba(0,0,0,0.6) 70%,rgba(0,0,0,0.85) 100%); }
.game-badge { position:absolute; top:10px; right:10px; background:rgba(22,163,74,0.95); backdrop-filter:blur(8px); color:#fff; font-size:0.625rem; font-weight:700; padding:4px 8px; border-radius:999px; text-transform:uppercase; letter-spacing:0.05em; }
.game-card-footer { position:absolute; bottom:0; left:0; right:0; padding:12px; display:flex; align-items:center; gap:8px; }
.game-logo-wrap { width:36px; height:36px; background:rgba(255,255,255,0.95); backdrop-filter:blur(8px); border-radius:10px; display:flex; align-items:center; justify-content:center; flex-shrink:0; padding:5px; }
.game-logo-img { width:100%; height:100%; object-fit:contain; }
.game-card-name { font-size:0.8125rem; font-weight:700; color:#fff; line-height:1.2; text-shadow:0 1px 4px rgba(0,0,0,0.8); }
.game-card-hint { font-size:0.6875rem; color:#22c55e; margin-top:2px; text-shadow:0 1px 2px rgba(0,0,0,0.5); }
.game-card-glow { position:absolute; inset:0; opacity:0; background:radial-gradient(ellipse at center,rgba(22,163,74,0.15) 0%,transparent 70%); transition:opacity 0.3s; pointer-events:none; }
.game-card-skeleton { aspect-ratio:3/4; border-radius:16px; background:linear-gradient(90deg,var(--muted,#f3f4f6) 25%,#e9ebee 50%,var(--muted,#f3f4f6) 75%); background-size:200% 100%; animation:shimmer 1.5s infinite; }
@keyframes shimmer { 0%{background-position:200% 0} 100%{background-position:-200% 0} }
.selected-game-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:16px; padding:14px 16px; background:rgba(22,163,74,0.05); border:1px solid rgba(22,163,74,0.2); border-radius:14px; }
.back-game-btn { display:flex; align-items:center; gap:4px; font-size:0.8125rem; font-weight:600; color:#16a34a; background:none; border:none; cursor:pointer; padding:0; }
.back-game-btn:hover { color:#15803d; }
.selected-game-info { display:flex; align-items:center; gap:10px; }
.selected-game-logo { width:40px; height:40px; object-fit:contain; background:rgba(255,255,255,0.5); border-radius:10px; padding:6px; border:1px solid var(--border,#e5e7eb); }
.selected-game-name { font-size:0.9rem; font-weight:700; color:var(--foreground,#111827); }
.selected-game-sub { font-size:0.75rem; color:var(--muted-foreground,#6b7280); }
.info-banner { display:flex; align-items:center; gap:10px; padding:12px 14px; background:#eff6ff; border:1px solid #bfdbfe; border-radius:12px; margin-bottom:16px; }
...
.product-card:disabled,.product-inactive { opacity:0.5; cursor:not-allowed; }
.product-icon { width:36px; height:36px; background:rgba(22,163,74,0.1); border-radius:10px; display:flex; align-items:center; justify-content:center; margin-bottom:10px; }
.ticket-icon { color:#16a34a; }
.product-name { font-size:0.75rem; font-weight:600; color:var(--foreground,#374151); line-height:1.3; margin-bottom:8px; }
.product-price { font-size:0.9375rem; font-weight:800; color:#16a34a; }
.product-unavail { font-size:0.6875rem; color:#ef4444; margin-top:2px; }
.product-skeleton { height:120px; border-radius:14px; background:linear-gradient(90deg,var(--muted,#f3f4f6) 25%,#e9ebee 50%,var(--muted,#f3f4f6) 75%); background-size:200% 100%; animation:shimmer 1.5s infinite; }
.empty-state { text-align:center; padding:60px 20px; }
.empty-icon { color:var(--muted-foreground,#9ca3af); margin:0 auto 16px; display:block; }
.empty-title { font-size:1rem; font-weight:600; color:var(--muted-foreground,#6b7280); margin-bottom:6px; }
.empty-sub { font-size:0.875rem; color:var(--muted-foreground,#9ca3af); }
</style>