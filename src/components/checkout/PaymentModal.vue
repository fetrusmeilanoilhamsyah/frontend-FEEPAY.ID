      ...
      >
        <div class="modal-content">
          ...
          <div class="product-info">
            ...
            <!-- Nomor HP diformat rapi -->
            <div class="info-row">
              <span class="info-label">{{ customerLabel }}</span>
              <span class="info-value font-mono">{{ formattedPhone }}</span>
            </div>
...
// ── Email: load dari localStorage kalau tidak ada props ───────────────────
...
watch(() => props.product, (val) => {
  if (val && !props.customerEmail) {
    // Restore saved email, jangan reset kalau ada
...
// ── Format nomor HP: 0831-7713-4548 ──────────────────────────────────────
const formattedPhone = computed(() => {
  const num = (props.customerData || '').replace(/\D/g, '')
  if (num.length <= 4) return num
  if (num.length <= 8) return `${num.slice(0, 4)}-${num.slice(4)}`
  if (num.length <= 12) return `${num.slice(0, 4)}-${num.slice(4, 8)}-${num.slice(8)}`
...
// ── Operator logo dari /logos/operators/ ──────────────────────────────────
const OPERATORS = [
